...
function Invoke-AuthenticatedCloudRunCanary {
    ...
    if (-not [string]::IsNullOrWhiteSpace($InvokerAccount)) {
        $tokenArgs += "--impersonate-service-account=$InvokerAccount"
    }
    $tokenArgs += @("--audiences=$Audience", '--quiet')
    $rawToken = Invoke-Gcloud -Capture -Arguments $tokenArgs
    $identityToken = @([regex]::Split([string]$rawToken, '\r?\n') |
        Where-Object { $_.Trim().Split('.').Count -eq 3 })
    $identityToken = if ($identityToken.Count -gt 0) { $identityToken[-1].Trim() } else { '' }
    ...
    try {
        ...
        $solveResponse = Invoke-WebRequest `
            ...
            -TimeoutSec 60
        $solveContentText = if ($solveResponse.Content -is [byte[]]) {
            [Text.Encoding]::UTF8.GetString($solveResponse.Content)
        } else {
            [string]$solveResponse.Content
        }
        $terminalLines = @(
            [regex]::Split($solveContentText, '\r?\n') |
                Where-Object { $_.Trim().StartsWith('@@TKB_RESULT@@') }
        )
        if ($terminalLines.Count -ne 1) {
            Write-Host "Solve Response Preview: $($solveContentText.Substring(0, [Math]::Min(500, $solveContentText.Length)))"
...
try {
    ...
    Invoke-AuthenticatedCloudRunCanary `
        ...
        -ExpectedRevision $newRevision `
        -ExpectedDigest $solverDigest `
        -InvokerAccount $InvokerServiceAccount
