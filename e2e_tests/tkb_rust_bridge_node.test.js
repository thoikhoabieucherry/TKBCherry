...
function createProgressDocument(clock){
  ...
  const timer = makeNode("autoSortTimerVal", {textContent:"00:00"});
  ...
  return {document, nodes, events, button, wrap, fill, pct, track, label, metric, timer, home, duration};
}

function installRealPlannerProgressUi(window, progress, runtime = {}){
...
test("progress appears for Play and hides after a successful terminal state", () => {
  ...
  // The running-state label is intentionally user-facing and stable from the
  // first frame.  "Chuẩn bị" was the pre-FET-first wording; the bridge now
  // uses the same explicit status as the scheduler itself.
  assert.match(primeBody, /setProgress\(0,\s*"Đang sắp xếp\.\.\."\s*,\s*\{replaceLocalPercent:true,\s*phase:"preparing"\}\)/);
...
test("automatic sorting paints progress and slices validation before starting the solver", () => {
  const bodyStart = BRIDGE_SOURCE.indexOf("async function bridgeSapXepTuDongAll");
  const bodyEnd = BRIDGE_SOURCE.indexOf("const autoSortButton", bodyStart);
  assert.ok(bodyStart >= 0 && bodyEnd > bodyStart, "automatic bridge body must be present");
  const body = BRIDGE_SOURCE.slice(bodyStart, bodyEnd);
...
test("a fresh Play shows an immediate preparation frame before timed progress", () => {
  ...
  assert.equal(visibleProgressCalls[0]?.percent, 0);
  assert.equal(visibleProgressCalls[0]?.label, "Đang sắp xếp...");
  assert.equal(visibleProgressCalls[0]?.at, clock.now());
  const initialFrameCount = visibleProgressCalls.length;

  clock.advance(999);
  assert.equal(progress.wrap.hidden, false);
  assert.equal(visibleProgressCalls.length, initialFrameCount);
...
test("an unchanged refinement ends cleanly without blocking a later Play", () => {
  const body = BRIDGE_SOURCE.slice(
    BRIDGE_SOURCE.indexOf("async function bridgeSapXepTuDongAll(options)"),
    BRIDGE_SOURCE.indexOf("const autoSortButton", BRIDGE_SOURCE.indexOf("async function bridgeSapXepTuDongAll(options)"))
  );
...
test("ordinary bridge authorization allows all four focused modes but keeps generic gaps operations-only", async () => {
  ...
  for(const mode of ["optimize_singletons", "optimize_sessions", "optimize_gap1", "optimize_gap2"]){
  ...
  const result = await window.sapXepTheoCheDo("optimize_gaps");
...
test("Quick progress follows scheduled periods and does not advance with time", () => {
  ...
  const stopButton = installRealPlannerProgressUi(window, progress, clock);
  progress.button.disabled = true;
  window.__TKB_SOLVE_UI_BUSY = true;
  const plan = hooks.applyRequestedSolveModeToPlan(
    hooks.buildAutomaticAutoSortPlan(data),
  ...
  assert.equal(progress.timer.textContent, "00:00");
  ...
  assert.equal(progress.timer.textContent, "00:30");
  assert.equal(progress.metric.textContent, "1/2 ti\u1ebft");
  hooks.recordBackendLiveProgress({
    protocol:"tkb-reference-solver-progress-v1",
  ...
  assert.equal(progress.metric.textContent, "2/2 ti\u1ebft");
  assert.equal(progress.timer.textContent, "00:30");
...
test("Hybrid focused requests use one canonical mode mapping and clamped Cloud Run budgets", () => {
  const data = installCompleteSinglePeriodSchedule(makeData(1));
  const {hooks} = loadBridge(data);
  const mapped = mode => hooks.settingsForSolveRequestMode(mode, {
    routing_mode:"serverless_only",
    ui_hybrid_executor:"cloud_run"
  }, data).settings;

  const singleton = mapped("optimize_singletons");
  assert.equal(singleton.ui_requested_solve_mode, "optimize_singletons");
  assert.equal(singleton.optimization_focus, "singletons");
  assert.equal(Object.hasOwn(singleton, "optimization_gap_target"), false);

  const sessions = mapped("optimize_sessions");
  assert.equal(sessions.ui_requested_solve_mode, "optimize_sessions");
  assert.equal(sessions.optimization_focus, "sessions");

  const gap2 = mapped("optimize_gap2");
  assert.equal(gap2.ui_requested_solve_mode, "optimize_gap2");
  assert.equal(gap2.optimization_focus, "gaps");
  assert.equal(gap2.optimization_gap_target, "gap2");

  const gap1 = mapped("optimize_gap1");
  assert.equal(gap1.ui_requested_solve_mode, "optimize_gap1");
  assert.equal(gap1.optimization_focus, "gaps");
  assert.equal(gap1.optimization_gap_target, "gap1");

  const standardScope = hooks.backendHybridDedupeScope(gap2, data);
  assert.match(standardScope, /::hybrid:gaps:gap2:standard:/);
  const deepScope = hooks.backendHybridDedupeScope(Object.assign({}, gap2, {
    ui_hybrid_deep_optimize:true
  }), data);
  assert.match(deepScope, /::hybrid:gaps:gap2:deep:/);
  assert.notEqual(deepScope, standardScope, "Deep and Standard must never share a Cloud Run job");

  const automatic = mapped("auto");
  assert.equal(automatic.ui_requested_solve_mode, "automatic");
  assert.equal(automatic.optimization_focus, "automatic");
  assert.equal(Object.hasOwn(automatic, "optimization_gap_target"), false);

  const standard = hooks.applyHybridCloudRunBudget({ui_hybrid_executor:"cloud_run"});
  assert.equal(standard.ui_hybrid_cloud_run_budget_seconds, 60);
  assert.equal(standard.optimization_time_limit_seconds, 60);
  assert.equal(standard.backend_deadline_ms, 60_000);

  const deep = hooks.applyHybridCloudRunBudget({
    ui_hybrid_executor:"cloud_run",
    ui_hybrid_deep_optimize:true
  });
  assert.equal(deep.ui_hybrid_cloud_run_budget_seconds, 180);
  assert.equal(deep.optimization_time_limit_seconds, 180);
  assert.equal(deep.backend_deadline_ms, 180_000);

  const intent = hooks.hybridCloudRunInvocationSettings({
    ...
    settings:{
      ...
      ui_hybrid_executor:"cloud_run",
      ui_hybrid_deep_optimize:true,
      optimization_focus_mode:"stale_alias",
      optimization_focus:"stale_alias"
    }
  });
  assert.equal(intent.routing_mode, "serverless_only");
  assert.equal(intent.ui_hybrid_executor, "cloud_run");
  assert.equal(intent.ui_hybrid_deep_optimize, true);
  assert.equal(intent.ui_hybrid_cloud_run_requested, true);
  assert.equal(
    hooks.hybridCloudRunInvocationSettings({
      fromHybridCaller:true,
      mode:"auto",
      settings:{ui_hybrid_executor:"cloud_run"}
    }),
    null,
    "Auto must never acquire a Cloud Run Hybrid route"
  );
});

test("planner keeps Auto FET-first and requires a normalized applied Cloud Run candidate", () => {
  const start = PLANNER_SOURCE.indexOf("async function executeDirectFastSchedule");
  const end = PLANNER_SOURCE.indexOf("function countMon(", start);
  assert.ok(start >= 0 && end > start, "fast scheduler function is missing");
  const body = PLANNER_SOURCE.slice(start, end);

  assert.match(body, /const useHybridOptimization = isOptimizeMode[\s\S]*?isHybridModeEnabled\(\) \|\| options\?\.forceCloudRun === true/);
  assert.doesNotMatch(body, /if\(isHybridModeEnabled\(\)\)\{/);
  assert.match(body, /normalized\.ok === true && normalized\.applied === true && normalized\.tkb/);
  assert.doesNotMatch(body, /backendRes && \(backendRes\.tkb \|\| data\.tkb\)/);
  assert.match(body, /Cloud Run thất bại — lịch chưa thay đổi/);
  assert.match(body, /Đang chuẩn bị tối ưu \$\{label\} trên máy này/);
  assert.match(body, /Tối ưu trên máy này \(Cloud Run không khả dụng\)/);
  assert.match(body, /ui_hybrid_deep_optimize: options\?\.deepOptimize === true/);
  assert.match(body, /validateFetCandidateHardConstraints\(data, normalized\.tkb\)/);
  assert.doesNotMatch(body, /optimization_focus_mode: options\.mode/);
  assert.match(body, /validateFetCandidateHardConstraints\(data, msg\.tkb/);
  assert.match(body, /__LATEST_OPTIMIZE_TKB = null;/);
  assert.doesNotMatch(PLANNER_HTML, /runPlannerDeepOptimize|Tối ưu sâu|Cloud Run/);
});
