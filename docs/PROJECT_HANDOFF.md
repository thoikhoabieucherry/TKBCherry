...
- Cung cố đường `exact_v2`: allocation master giờ được dùng như **cận dưới
  ...
  tối ưu thay vì phụ thuộc hoàn toàn vào best-bound của model lớn.
- Strict UI adapter không còn gọi `_school_data_with_lesson_block_caps`; tức
  `lessonBlocks.Min` không được tự nâng `Giới hạn/Max`. Solver V2 thêm
  precheck `lesson_block_min_conflicts_with_upper_bound` cho các mâu thuẫn
  `Min` vs cận trên/số tiết tuần và giữ nguyên lịch cũ.
- UI planner tiếp tục chỉ hiện một nút Sắp xếp. Các alias cũ
  `sapXepTheoCheDo`/`deepOptimizeTheoCheDo` bị quy về `automatic`; merge
  settings ở `phanmon.js` và `tkb-rust-bridge.js` ép lại `solver_algorithm=
  exact_v2`, tắt retry/optimize phụ, best-effort và các mode FET cũ. Stop
  button đổi nhãn thành “Dừng sắp xếp”; cache bump `exact-v2-v2`.
- Verification local: `node --check web/pages/phanmon.js`, `node --check
  web/pages/tkb-rust-bridge.js`, `python -m unittest
  solver_runtime.tests.test_exact_v2 solver_runtime.tests.test_model_plan_contract`
  bằng venv `C:\Users\Love\.cache\tkb-test-venv` => **11 tests OK, 1 skipped**.
  Smoke trực tiếp Solver V2 request 2 tiết => `ok=True`, `2/2`, singleton 0,
  gap `{0:1}`, sessions `OPTIMAL`.
- `start.py --foreground` trên port 1010 bị chặn vì port đang do
  `C:\Users\Love\Documents\Codex\ProjectX\TKBCherryAnti\...\tkb_rust_api.exe`
  giữ. Đã khởi động local repo hiện tại bằng `start.py --foreground --port
  1011 --no-browser`; `/api/health` OK và `appRoot` trỏ đúng
  `TKBCherryCodex`.
- Chưa deploy VPS/Cloud Run. Các file FET đang dirty từ phiên khác được giữ
  nguyên, không stage/không revert.

## 2026-08-18 Solver V2 integrated CP-SAT — local only, chưa deploy

- Theo yêu cầu mới, đường xếp TKB được dựng lại độc lập trong
  `solver_runtime/src/tkb_exact_v2/solver.py`; không gọi FET Web Worker, các
  operator local-search hay các mode `optimize_*`.
- Mô hình dùng pattern khối cho từng phân công và pattern tải giáo viên 0 hoặc
  >=2 tiết với Gap2+ hard zero. Pha 1 chứng minh tổng `teacher_sessions` nhỏ
  nhất; pha 2 khóa kết quả đó và chứng minh Gap1 nhỏ nhất. Chỉ `OPTIMAL` hoặc
  objective==best_bound mới được đóng gói thành công.
- Decoder Solver V2 gọi `build_school_data_from_ui(..., strict_constraints=True)`;
  `lessonBlocks.Min` không còn bị sanitize/hạ. Tiền kiểm báo rõ mã
  `fixed_lessons_exceed_upper_bound` khi ô cố định vượt Giới hạn/Max — đúng
  nghĩa Giới hạn là cận trên, không phải bắt buộc chạm mức.
- `solve_stdio.py` đã route `settings.solver_algorithm=exact_v2`; lỗi vô nghiệm/
  chưa chứng minh trả `exact_v2_no_solution`, không best-effort và không làm
  mất lịch cũ. UI `sapxep.html` bỏ nút Tối ưu, NEW, Trọn gói và không tải FET
  engine; `phanmon.js` định tuyến nút Sắp xếp duy nhất sang backend V2.
- Test mới `solver_runtime/tests/test_exact_v2.py`: **5/5**; model-plan test
  vẫn xanh. Smoke stdio request 2 tiết trả đủ 2/2, singleton 0, Gap2 0, hai
  chứng chỉ OPTIMAL. Fixture `scratch/school_95671c41791.json` được kiểm tra
  read-only và phát hiện đúng 12 xung đột ô cố định vượt Max, chưa áp gì.
- Chưa deploy VPS/Cloud Run và chưa thay đổi SQLite/user timetable. Bước kế
  tiếp: khởi động `start.py`, smoke `/api/health` + `/api/solve-data`, sau đó
  benchmark fixture lớn; chỉ canary sau khi đạt các gate trong
  `docs/SOLVER_V2_PLAN.md`.
- Smoke sau rebuild đã xác nhận `backend=exact-v2-cp-sat`, `name=
  tkb_exact_v2_integrated_pattern_cp_sat`, `phase=exact_v2` và nghiệm mẫu
  `2/2`, singleton `0`, Gap2 `0`, sessions/Gap1 đều `OPTIMAL`.
- Benchmark read-only fixture `school_95671c41791.json` (1.530 tiết, tháo
  cờ fixed không hợp lệ chỉ trong bản sao bộ nhớ) chưa chứng minh được pha
  sessions trong 60 giây và 180 giây; Solver V2 trả
  `sessions_optimum_not_proven`, không candidate được publish. Đây là blocker
  hiệu năng/mô hình còn mở, không phải lý do để nới rule hoặc deploy.
- `cargo check --manifest-path rust_api/Cargo.toml` xanh khi trỏ SQLite vendor;
  `cargo test` mặc định trên máy Windows vẫn lỗi linker GNU `cannot find
  -lsqlite3` nếu không truyền cùng biến môi trường vendor. Python focused
  suites hiện xanh: exact V2 `5/5`, model-plan + result-contract `188/188`
  (2 skip).
