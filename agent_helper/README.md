...
3. Mở `TKBCherryAgent.exe` mới. Nếu một bản cũ đang chạy đúng tại đường dẫn Agent đã cài, bản mới chỉ dừng cây tiến trình đó, thay file theo kiểu atomic rồi tự khởi động lại; không cần người dùng tắt thủ công. Một file trùng hệt bản đang cài được dùng lại mà không kill/copy lại. Lần đầu Agent tự chép bản chính vào `C:\TKBCherryAgent\TKBCherryAgent.exe`; nếu Windows không cho ghi vào ổ C thì dùng `%LOCALAPPDATA%\TKBCherry\Agent\`. Sau đó Agent tự chuyển sang **ON** rồi ẩn ngay xuống khay thông báo, không bật cửa sổ. Logo TKBCherry được dùng cho EXE, cửa sổ, thanh tác vụ và khay hệ thống; Agent cũng tự đăng ký chạy cùng lần đăng nhập của tài khoản Windows hiện tại mà không cần quyền quản trị.
...
File phát hành cho web là `agent_helper\dist\TKBCherryAgent-Windows.zip`. Archive này được script kiểm tra bắt buộc chỉ có một entry `TKBCherryAgent.exe` ở root. EXE dùng PyInstaller `onefile`, chế độ cửa sổ; runtime solver được biên dịch thành bytecode tối ưu và bỏ file `.py` thô, sau đó EXE được nén bằng UPX 5.2.0 `--best --lzma --force`. Đây là giảm lộ source và nén gói, không được xem là bảo vệ tuyệt đối trước reverse engineering. Vì EXE tự chứa NumPy, SciPy và OR-Tools nên lần mở đầu có thể chậm hơn vài giây trong lúc giải nén an toàn vào thư mục tạm. Build chỉ thành công sau khi bản onedir, onefile trước/sau UPX tạo được cửa sổ Tk ẩn thật, UPX `-t` hợp lệ và solver-child trả đúng protocol. Gói không kèm token, mật khẩu, CMD hay PowerShell.
