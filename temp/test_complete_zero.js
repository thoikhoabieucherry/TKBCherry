...
// Test iteratively
console.log("=== RUNNING ENHANCED CRUSHER UNTIL CONVERGENCE ===");
...
for(let iter = 1; iter <= 20; iter++) {
  ...
  if(r1 && engine.compareMetrics(r1, m, "optimize_gap2") < 0) {
    m = { ...r1 };
    improved = true;
    console.log(`[Iter ${iter} - Extreme Crusher] Gap2: ${m.soBuoiTrong2}, Singletons: ${m.soBuoiDay1}, Sessions: ${m.tsBuoiDay}, Gap1: ${m.soBuoiTrong1}`);
  }
  
  const r2 = engine.tryBorrowLessonFromRichSessions(m, m, "optimize_gap2");
  if(r2 && engine.compareMetrics(r2, m, "optimize_gap2") < 0) {
    m = { ...r2 };
    improved = true;
    console.log(`[Iter ${iter} - Borrow Rich] Gap2: ${m.soBuoiTrong2}, Singletons: ${m.soBuoiDay1}, Sessions: ${m.tsBuoiDay}, Gap1: ${m.soBuoiTrong1}`);
  }

  const r3 = engine.tryBlockShiftAndGapResolution(m, m, "optimize_gap2");
  if(r3 && engine.compareMetrics(r3, m, "optimize_gap2") < 0) {
    m = { ...r3 };
    improved = true;
    console.log(`[Iter ${iter} - Block Shift] Gap2: ${m.soBuoiTrong2}, Singletons: ${m.soBuoiDay1}, Sessions: ${m.tsBuoiDay}, Gap1: ${m.soBuoiTrong1}`);
  }
  
  if(!improved) break;
}

console.log("\nFINAL CONVERGED METRICS:", m);
