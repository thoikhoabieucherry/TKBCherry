...
def rust_build_env(base: dict[str, str] | None = None) -> dict[str, str]:
    """Return a build environment that can link SQLite on the bundled GNU toolchain."""
    env = (base or os.environ).copy()
    if os.name == "nt" and RUST_SQLITE_IMPORT_LIB.exists():
        env.setdefault("SQLITE3_LIB_DIR", str(RUST_SQLITE_VENDOR_DIR))
        env.setdefault("SQLITE3_DYNAMIC", "1")
    return env
