...
const gvKey = Array.from(opt.teacherGrid.keys()).find(k => k.toLowerCase() === 'sđ.linh');
const grid = opt.teacherGrid.get(gvKey);
console.log(`Teacher ${gvKey} grid (6 days x 2 sessions x 5 periods):`);
for(let d = 0; d < 6; d++){
  const sang = [];
  const chieu = [];
  for(let p = 0; p < 5; p++){
