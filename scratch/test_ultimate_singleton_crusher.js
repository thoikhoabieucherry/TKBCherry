...
// Let's implement the Super Singleton Crusher combining all 5 techniques:
function runSuperSingletonCrusher(engine){
  const t0 = Date.now();
  ...
  for(let round = 0; round < 15; round++){
    ...
    // PASS A: Same-Teacher Same-Class Pairing
    const resConsolidate = engine.tryConsolidateTeacherSingletons(bestM, bestM);
    if(resConsolidate && resConsolidate.soBuoiDay1 < bestM.soBuoiDay1){
      bestM = { ...resConsolidate };
      roundImp = true;
    }

    // PASS B: Vacate Singletons via 2-way and 3-way cycles
    ...
    // PASS C: Inbound Reinforce from any session where teacher has >= 3 periods
    for(const tKey of teacherList){
      const tGrid = engine.teacherGrid.get(tKey);
      if(!tGrid) continue;
  ...
  console.log(`Crusher finished in ${Date.now() - t0}ms:`, bestM);
  return bestM;
}

runSuperSingletonCrusher(opt);
