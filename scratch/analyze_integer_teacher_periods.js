...
for(const [k, gv] of Object.entries(schoolData.pccmMatrix || {})){
  if(!gv) continue;
  const count = parseInt(schoolData.pccmTietMatrix?.[k] || 1, 10);
...
console.log("=== ALL TEACHERS SORTED BY TOTAL PERIODS ===");
for(const [gv, total] of Array.from(teacherTotal.entries()).sort((a,b) => a[1] - b[1])){
  ...
  console.log(`- ${gv}: total ${total} periods | Off: ${JSON.stringify(off)}`);
}
