...
// Step 2: Global BFS 3-Way Cycle Augmented Path
function solveSingletonsViaStrict3Way(engine, maxPasses = 50){
  const DAYS = 6;
  const SESSIONS = 2;
  const PERIODS = 5;
  ...
  for(let pass = 0; pass < maxPasses; pass++){
    ...
    for(const sing of singletons){
      ...
      for(let s2 = 0; s2 < 60; s2++){
          ...
          if(r1.possible && r1.conflicts.length === 0 && r2.possible && r2.conflicts.length === 0){
            ...
            if(engine.isLessonBlockSafe(act1, act2)){
              ...
              if(m.soBuoiDay1 < bestM.soBuoiDay1 || (m.soBuoiDay1 === bestM.soBuoiDay1 && m.tsBuoiDay < bestM.tsBuoiDay)){
                ...
                passImproved = true;
                resolved = true;
                console.log(`[Strict 2-Way] ${bestM.soBuoiDay1}`);
                break;
              }
            }
            engine.unplaceActivity(act1.id);
        ...
        // Try 3-way cycle (act1 -> s2, act2 -> s3, act3 -> s1)
          ...
          if(r1.possible && r1.conflicts.length === 0 &&
            ...
            if(engine.isLessonBlockSafe(act1, act2, act3)){
              ...
              if(m.soBuoiDay1 < bestM.soBuoiDay1 || (m.soBuoiDay1 === bestM.soBuoiDay1 && m.tsBuoiDay < bestM.tsBuoiDay)){
                ...
                passImproved = true;
                resolved = true;
                console.log(`[Strict 3-Way] ${bestM.soBuoiDay1}`);
                break;
              }
            }
            engine.unplaceActivity(act1.id);
...
solveSingletonsViaStrict3Way(opt, 50);
console.log("After Strict 3-Way:", opt.evaluateMetrics());
