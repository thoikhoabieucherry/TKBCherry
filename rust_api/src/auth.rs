...
pub fn ensure_super_admin(db: &Db) {
    ...
    let env_password = env::var("TKB_SUPER_PASSWORD").unwrap_or_default();
    let has_env_password = !env_password.trim().is_empty();
    ...
    let needs_bootstrap = !users.contains_key(SUPER_ID) || current_hash.is_empty();

    if !needs_bootstrap && has_env_password && verify_password(env_password.trim(), current_hash) {
        return;
    }
    if !needs_bootstrap && !has_env_password {
        return;
    }

    let password = if has_env_password {
        env_password
    } else {
