...
// Schedule state cloner for candidate validation and local snapshots
function clonePlannerScheduleState(data){
...
function restorePlannerScheduleState(data, snapshot){
...
const cloneHybridScheduleState = clonePlannerScheduleState;
const restoreHybridScheduleState = restorePlannerScheduleState;
...
async function inspectFetCandidateHardConstraints(data, candidateTkb){
  ...
  const snapshot = clonePlannerScheduleState(data);
  try{
...
async function executeDirectFastSchedule(options = {}){
  try{
    ...
    // Run local FET engine on background Web Worker
    ...
    if(FetEngineClass){
      const directFetIncumbent = clonePlannerScheduleState(data);
      ...
      if(candidateValidation.ok !== true){
        restorePlannerScheduleState(data, directFetIncumbent);
...
function renderStatsBox(){
  const box = document.getElementById("statsBox");
  if(!box) return;
  const isSolving = (window.__ACTIVE_TKB_FET_WORKER != null) || window.__TKB_SOLVE_UI_BUSY === true;
...
function updateStatsBoxLiveProgress(snapshot){
  ...
  const box = document.getElementById("statsBox");
  if(!box) return false;
  const solving = (window.__ACTIVE_TKB_FET_WORKER != null) || window.__TKB_SOLVE_UI_BUSY === true;
...
function setAutoSortProgress(percent, label, details){
  ...
  const btn = document.getElementById("btnStopAutoSort");
  if(btn && !window.__AUTO_SORT_STOP_REQUESTED && n < 100){
    btn.disabled = false;
    setAutoSortStopAccessibleState(false);
  }
}
...
function isAutoSortRunningForNavigation(){
  return (window.__ACTIVE_TKB_FET_WORKER != null) || window.__TKB_SOLVE_UI_BUSY === true;
}
...
async function saveAndBack(){
  ...
  try{
    if(isAutoSortRunningForNavigation()){
      try{
        _setStatus("Đang có lượt xếp chạy trên máy; vui lòng bấm Dừng trước khi quay lại.", "info");
      }catch(_){ }
    }
