    ...
    try{
      ...
      if(urlSid || urlSchool){
        ...
        if(directSid){
          ...
          url.searchParams.delete("school");
          history.replaceState(null, "", url.toString());
          const label = (directSid === "default") ? "Default" : (getSchoolName(directSid) || localStorage.getItem("TKB_LAST_SCHOOL_LABEL") || directSid);
          localStorage.setItem("TKB_LAST_SCHOOL", directSid);
          localStorage.setItem("TKB_LAST_SCHOOL_LABEL", label);
        }
        return;
      }
