...
(function(){
  ...
  function releaseExistingViolationsAfterSave(){
    try{
      if(typeof window.releaseConstraintViolatingLessons === 'function'){
        return Number(window.releaseConstraintViolatingLessons(D()) || 0);
      }
    }catch(e){
  ...
  function quickTkbSignature(){
    ...
    try{
      const m = model();
      const tkbRef = D().tkb;
      const tkbLen = tkbRef ? Object.keys(tkbRef).length : 0;
      return String(__cacheRev) + '|' + String(tkbLen) + '|' + String(m.meta?.updatedAt || '') + '|' +
