(function(){
  ...
  // This card is deliberately separate from the legacy infrastructure card.
  // It consumes only the aggregate-only telemetry route; no timetable or
  // account identifiers ever enter this renderer.
  function initSolverTelemetryCard(){
    const card = document.getElementById("solverTelemetryCard");
    const api = window.TKBAuthApi;
    if(!card || card.dataset?.aggregateOnly !== "true" || !api || typeof api.getAuthHeaders !== "function" || typeof window.fetch !== "function") return;

    const byId = id => document.getElementById(id);
    const windowInput = byId("solverTelemetryWindow");
    const reloadButton = byId("btnReloadSolverTelemetry");
    const summary = byId("solverTelemetrySummary");
    const body = byId("solverTelemetryBody");
    const provenance = byId("solverTelemetryProvenanceBody");
    const status = byId("solverTelemetryStatus");
    let loading = null;
    ...
    const escTelemetry = value => String(value == null ? "" : value).replace(/[&<>'"]/g, c => ({
      "&":"&amp;", "<":"&lt;", ">":"&gt;", "'":"&#39;", "\"":"&quot;"
    }[c]));
    ...
    const displayMs = value => {
      const number = finite(value);
      if(number == null) return "—";
      if(number < 1000) return `${Math.round(number)} ms`;
      return `${(number / 1000).toLocaleString("vi-VN", {maximumFractionDigits:1})} s`;
    };
    const displayUsd = value => {
      const number = finite(value);
      return number == null ? "—" : `$${number.toLocaleString("en-US", {minimumFractionDigits:2, maximumFractionDigits:4})}`;
    };
    const displayKey = (kind, key) => {
      const raw = String(key || "unknown");
      const labels = {
        fet_web_worker:"FET Web Worker",
        cloud_run:"Cloud Run CP-SAT",
        vps:"VPS",
        automatic:"Tự động",
        singletons:"1 tiết/buổi",
        sessions:"Buổi dạy",
        "gaps:gap2":"2 tiết trống",
        "gaps:gap1":"1 tiết trống",
        local:"Local",
        standard:"Standard",
        deep:"Deep Optimize"
      };
      return labels[raw] || `${kind}: ${raw}`;
    };
    const aggregateRows = payload => {
      const rows = [];
      const routed = Array.isArray(payload?.byExecutorFocusBudget) ? payload.byExecutorFocusBudget : [];
      if(routed.length){
        routed.forEach(item => {
          const aggregate = item && item.aggregate ? item.aggregate : null;
          if(!aggregate) return;
          const executor = displayKey("Executor", item.executor);
          const focusKey = item.gapTarget ? `${item.focus}:${item.gapTarget}` : item.focus;
          const focus = displayKey("Mục tiêu", focusKey);
          const budget = displayKey("Ngân sách", item.budgetKind);
          const seconds = displayNumber(item.budgetSeconds);
          rows.push({
            kind:"Executor / mục tiêu / ngân sách",
            key:`${executor} · ${focus} · ${budget} (${seconds} s)`,
            aggregate
          });
        });
        return rows;
      }
      const append = (kind, list) => {
        (Array.isArray(list) ? list : []).forEach(item => {
          const aggregate = item && item.aggregate ? item.aggregate : null;
          if(!aggregate || !item.key) return;
          rows.push({kind, key:String(item.key), aggregate});
        });
      };
      append("Executor", payload?.byExecutor);
      append("Mục tiêu", payload?.byFocus);
      append("Ngân sách", payload?.byBudget);
      return rows;
    };
    const renderSummary = payload => {
      if(!summary) return;
      const overall = payload?.overall || {};
      const runtime = overall.runtimeMs || {};
      const events = payload?.events || {};
      summary.innerHTML = [
        ["Lượt chạy", displayNumber(events.total ?? overall.total)],
        ["Hoàn tất", displayPct(overall.successRate)],
        ["Hard-valid", displayPct(overall.hardValidRate)],
        ["Đạt mục tiêu", displayPct(overall.targetReachedRate)],
        ["p50 / p95", `${displayMs(runtime.p50)} / ${displayMs(runtime.p95)}`],
        ["Deep rate", displayPct(payload?.deepRunRate)],
        ["Δ metric TB", displayNumber(overall.metric?.deltaAverage)],
        ["Chi phí ước tính", displayUsd(overall.estimatedCostUsd)]
      ].map(([label, value]) => `<div class="solver-telemetry-stat"><span>${escTelemetry(label)}</span><strong>${escTelemetry(value)}</strong></div>`).join("");
    };
    const renderRows = payload => {
      if(!body) return;
      const rows = aggregateRows(payload);
      if(!rows.length){
        body.innerHTML = `<tr><td colspan="9" class="portal-muted">Chưa có dữ liệu trong khoảng đã chọn.</td></tr>`;
        return;
      }
      body.innerHTML = rows.map(row => {
        const aggregate = row.aggregate || {};
        const runtime = aggregate.runtimeMs || {};
        const errors = Number(aggregate.failed || 0) + Number(aggregate.cancelled || 0) + Number(aggregate.timeout || 0);
        return `<tr>
          <td><strong>${escTelemetry(displayKey(row.kind, row.key))}</strong><small>${escTelemetry(row.kind)}</small></td>
          <td>${escTelemetry(displayNumber(aggregate.total))}</td>
          <td>${escTelemetry(displayPct(aggregate.successRate))}</td>
          <td>${escTelemetry(displayNumber(aggregate.noImprovement))}</td>
          <td>${escTelemetry(displayNumber(errors))}</td>
          <td>${escTelemetry(displayMs(runtime.p50))} / ${escTelemetry(displayMs(runtime.p95))}</td>
          <td>${escTelemetry(displayPct(aggregate.targetReachedRate))}</td>
          <td>${escTelemetry(displayNumber(aggregate.metric?.deltaAverage))}</td>
          <td>${escTelemetry(displayUsd(aggregate.estimatedCostUsd))}</td>
        </tr>`;
      }).join("");
    };
    const renderProvenance = payload => {
      if(!provenance) return;
      const cloud = Array.isArray(payload?.cloud) ? payload.cloud : [];
      if(!cloud.length){
        provenance.textContent = "Chưa có lượt Cloud Run trong khoảng đã chọn.";
        return;
      }
      provenance.innerHTML = cloud.map(item => {
        const profile = escTelemetry(item?.profile || "unknown");
        const revision = escTelemetry(item?.revision || "unknown");
        const digestRaw = String(item?.digest || "unknown");
        const digest = escTelemetry(digestRaw.length > 20 ? `${digestRaw.slice(0, 20)}…` : digestRaw);
        return `<div class="solver-telemetry-provenance-item"><span>${profile}</span><code>${revision}</code><code title="Digest đã rút gọn">${digest}</code></div>`;
      }).join("");
    };
    ...
    const load = async () => {
      ...
      const selectedWindow = ["24h", "7d", "30d"].includes(String(windowInput?.value || ""))
        ? String(windowInput.value)
        : "7d";
      setStatus("Đang tải telemetry solver…", "");
      loading = requestJson(`/api/admin/solver-telemetry?window=${encodeURIComponent(selectedWindow)}`, {
        headers:api.getAuthHeaders()
      }).then(payload => {
        renderSummary(payload);
        renderRows(payload);
        renderProvenance(payload);
        setStatus(`Đã cập nhật dữ liệu ${selectedWindow}.`, "success");
        return payload;
      }).catch(error => {
        setStatus("Không tải được telemetry solver: " + String(error?.message || error), "error");
        if(body) body.innerHTML = `<tr><td colspan="9" class="portal-muted">Không có dữ liệu.</td></tr>`;
        return null;
      }).finally(() => { loading = null; });
      return loading;
    };
    reloadButton?.addEventListener("click", () => load());
    windowInput?.addEventListener("change", () => load());
    load();
  }
